...
class FeedbackRepository {
  ...
  Future<Feedback> saveFeedback({required Feedback feedback}) async {
    ...
    final url = _getUrl();
    final response =
        await dio.post<String>(url, data: feedback.toJson());
    if (response.statusCode == 201 && response.data != null) {
      final newPerson =
          Feedback.fromJson(json.decode(response.data!) as Map<String, Object?>);
      return newPerson;
    } else {
      throw ApiException(
        response.statusCode ?? -1,
...
FeedbackRepository feedbackRepository(FeedbackRepositoryRef ref) =>
   FeedbackRepository(dio: ref.read(dioProvider));
